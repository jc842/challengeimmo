# Directives Éditoriales & Contraintes d'Environnement — challengeimmo.com

Ce document récapitule les directives d'environnement et de monétisation obligatoires pour toute création de nouvel article sur **challengeimmo.com**.
...
| :--- | :--- |
| **Domaine** | `challengeimmo.com` |
| **Tracker Analytics** | Matomo cookieless (`https://analytics.les4h.fr/`) |
| **Conteneur MTM** | `container_Hx1AEp9v.js` (Site ID: `55`) |
| **Adserver Revive** | `https://ads.les4h.fr/` (Éditeur ID: `103`) |
...
```html
<script type="text/javascript" src="https://analytics.les4h.fr/js/container_Hx1AEp9v.js" async defer></script>
```
...
<!-- 1. Encart Pub In-Content Obligatoire (Medium Rectangle 300x250 - Zone 631) -->
<div class="my-12 py-6 border-y border-stone-200/80 flex justify-center">
  <ReviveSlot slot="inContent" />
</div>

<!-- 2. Encart Pub Fin d'Article Obligatoire (Large Rectangle 336x280 - Zone 628) -->
...
| :--- | :--- | :---: | :--- |
| **`header`** | Leaderboard (728x90) | **630** | Haut de page / sous navigation |
| **`inContent`** | Medium Rectangle (300x250) | **631** | Milieu d'article contextuel |
| **`largeRectangle`** | Large Rectangle (336x280) | **628** | Fin d'article avant recommandations |
| **`mobileSticky`** | Mobile Banner (320x100) | **629** | Bandeau sticky bas d'écran mobile |
| **`halfPage`** | Half Page (300x600) | **632** | Sidebar desktop pour articles longs |
| **`skyscraper`** | Skyscraper (160x600) | **633** | Colonne latérale de listing / dossiers |

---

## 4. Checklist Avant Publication
- [ ] L'article utilise le Layout avec le tracker MTM actif (`Hx1AEp9v`).
