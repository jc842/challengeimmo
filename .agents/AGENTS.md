# Directives & Contraintes d'Environnement — challengeimmo.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **challengeimmo.com** (challengeimmo) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `challengeimmo.com` |
| **URL Cloudflare Pages** | `https://challengeimmo.pages.dev` |
| **Dépôt GitHub** | `jc842/challengeimmo` (branche principale : `main`) |
...
| **Matomo Site ID** | `55` |
| **Matomo MTM Container** | `container_Hx1AEp9v.js` (Container ID : `Hx1AEp9v`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `103` |
| **Zones Publicitaires** | Header (`630`), In-Content (`631`), Mobile Sticky (`629`), Large Rectangle (`628`), Half Page (`632`), Skyscraper (`633`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_Hx1AEp9v.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 631).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 628).
